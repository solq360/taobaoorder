...
function Main(){
	...
	function doHandle(){		
 		var body = "",
			count = 0;
		var tableDom = $$("li.td-item");
		var infoDom = $$("li.td-info");
		var priceDom = $$("li.td-price");
		...
		tableDom.each(function(i,dom){
			...
			var a = rowDom.find(".J_ItemPic");
			var titleDoms = a.find("a")
			var img = titleDoms.html().replace("src=","t=").replace("lazyload=","src=")
			.replace("src=\"//","src=\"http://");
			
				body+=formatCsv([
					img,
					titleDoms.attr("href"),
					titleDoms.attr("data-title"),
					
					price.find(".price-content").text(),
					
					info.find(".item-props").text().trim().replace(/修改/g,""),
				]);
